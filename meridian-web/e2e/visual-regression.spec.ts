...
test.describe('Visual-regression: Dashboard', () => {
  for (const [label, viewport] of Object.entries(VIEWPORTS)) {
    test(`matches the saved snapshot at ${label}`, async ({ page }) => {
      await page.setViewportSize(viewport);
...
test.describe('Visual-regression: Sign-in page', () => {
  for (const [label, viewport] of Object.entries(VIEWPORTS)) {
    test(`matches the saved snapshot at ${label}`, async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(`sign-in-${label}.png`, {
        fullPage: true,
        threshold: 0.02,
      });
    });
...
test.describe('Visual-regression: 404 page', () => {
  test('matches the saved snapshot for an unknown route', async ({ page }) => {
...
test.describe('Visual-regression: Mobile menu open', () => {
  test('matches the saved snapshot with the navigation menu expanded', async ({ page }) => {
